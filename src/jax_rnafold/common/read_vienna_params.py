"""A set of parsing utilities and data structures for reading and storing nearest neighbor parameters."""
...
def read(param_path: str = TURNER_2004, max_precompute: int = MAX_PRECOMPUTE,
         postprocess: bool = True, temp: float = CELL_TEMP) -> dict:
    """
    Reads a set of nearest neighbor parameters from an input file.
    ...
    Args:
      ...
      postprocess: If true, will postprocess parameters to be JAX-compatible.
      temp: The temperature in Kelvin.  Defaults to 310.15 K.

    Returns:
      A dictionary of nearest neighbor parameters.
    """
...
class NNParams:
    """
    A class for storing nearest neighbor parameters.

    Attributes:
      params_path: A file path to nearest neighbor parameters.
      max_precompute: The maximum sequence length to precompute for initiation costs and asymmetry costs.
      postprocess: If true, will postprocess parameters to be JAX-compatible.
      save_sp_hairpins_jax: If True, will save the special hairpins in a JAX-compatible format.
      temp: The temperature in Kelvin.  Defaults to 310.15 K.
    """
    def __init__(self, params_path: str = TURNER_2004,
                 max_precompute: int = MAX_PRECOMPUTE,
                 postprocess: bool = True,
                 save_sp_hairpins_jax: bool = False,
                 temp: float = CELL_TEMP
    ):
        ...
        self.params = read(params_path, max_precompute=max_precompute,
                           postprocess=postprocess, temp=self.temp)
